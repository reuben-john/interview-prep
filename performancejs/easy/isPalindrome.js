...
test(
  "Odd length palindrome returns true",
  t => t.is(isPalindrome("abcdcba")),
  true
);
test("Return false if not palindrome", t => t.is(isPalindrome("abcd")), false);
test(
  "Long sentence that is palindrome should be true",
  t => t.is(isPalindrome("A man a plan and a canal Panama")),
  true
);
